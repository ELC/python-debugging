# Topics to cover:
# Run with "Run Python File"
#   `& .../python.exe .../01_basic_workflow/ide_debugger.py`
# Run snippet with "Shift+Enter"
#   `& .../python.exe`
# Run with "Debug Python File"
#   `& '...\python.exe' '...\debugpy\launcher' '54405' '--' '...\01_basic_workflow\ide_debugger.py'`
#   Equivalent command: `pipenv run python -m debugpy --listen localhost:5678 --wait-for-client .\01_basic_workflow\ide_debugger.py`
# Add Hit Count breakpoint:
#  2 on L34
# Add Expression breakpoint:
#  b == 0 on L51
# Add Log Message breakpoint:
# "The result value is {ternary_result}" on L65
...
def multiply_reduce(a: int, b: int) -> int:
    return reduce(lambda x, y: x + a, range(b), 0)


reduce_result = multiply_reduce(a, b)
assert reduce_result == 6
